...
static u64 *kp_kpm_pgtable_entry(u64 pgd_addr, u64 va)
{
    ...
    if (pud_sect(*pud)) return (u64 *)pud;
    pmd = pmd_offset(pud, va);
    if (pmd_none(*pmd) || pmd_bad(*pmd)) return NULL;
    if (pmd_sect(*pmd)) return (u64 *)pmd;
    pte = pte_offset_kernel(pmd, va);
    return pte ? (u64 *)pte : NULL;
}

__attribute__((no_sanitize("cfi"))) static int kp_kpm_printk(const char *fmt, ...)
{
...
{
    uint32_t insn = value;
    return kp_patch_text(addr, &insn, sizeof(insn), KP_PATCH_TEXT_FLUSH_DCACHE | KP_PATCH_TEXT_FLUSH_ICACHE);
}
...
static int (*kp_kpm_kallsyms_on_each_symbol_fn)(int (*fn)(void *, const char *, struct module *, unsigned long),
                                                void *data);
...
#define KP_KPM_KFUNC_INIT(name) kf_##name = (typeof(kf_##name))kallsyms_lookup_name(#name)
#define KP_KPM_KFUNC_ENTRY(name) { "kf_" #name, (unsigned long)&kf_##name }
...
{
    if (!is_compat && kp_sys_call_table) return kp_sys_call_table[nr];
    return 0;
}
...
{
    uintptr_t addr = kp_kpm_syscalln_addr(nr, 0);
    if (addr) hook_unwrap((void *)addr, before, after);
}
...
int kp_kpm_symbols_init(void)
{
    ...
    if (kallsyms_lookup_name("__arm64_sys_openat")) kp_kpm_has_syscall_wrapper = 1;

    logki("kpm stack facts: stack_in_task=%d stack_end=%d has_syscall_wrapper=%d\n", kp_kpm_stack_in_task_offset,
          kp_kpm_stack_end_offset, kp_kpm_has_syscall_wrapper);

    logki("kpm compatibility symbol table ready (%zu symbols) printk=%px "
          "kallsyms_lookup_name=%px kf_strncat=%px kf_strcmp=%px\n",
          ARRAY_SIZE(kp_kpm_symbols), kp_kpm_printk_fn, kp_kpm_kallsyms_lookup_name_fn, kf_strncat, kf_strcmp);
    return 0;
}
...
unsigned long kp_kpm_symbol_lookup(const char *name)
{
    ...
    for (i = 0; i < ARRAY_SIZE(kp_kpm_symbols); i++) {
        if (!strcmp(name, kp_kpm_symbols[i].name)) return kp_kpm_symbols[i].addr;
    }
    return 0;
}

...
## What is KPM-Manager?
KPM-Manager is a root solution similar to Magisk or KernelSU that unites the best of both.
...
## What's the difference between KPM-Manager and Magisk?
- Magisk modifies the init system with a patch in your boot image's ramdisk, while KPM-Manager patches the kernel directly.


## KPM-Manager vs KernelSU
- KernelSU requires the source code for your device's kernel which is not always provided by the OEM. KPM-Manager works with just your stock `boot.img`.


## KPM-Manager vs Magisk, KernelSU
- KPM-Manager allows you to optionally not modify SELinux, this means that the APP thread can be rooted, libsu and IPC are not necessary.
...
For more information, see [How to write a KPM](https://github.com/yervant7/KPM-Manager/blob/main/doc/module.md)


## Relationship between KPM-Manager and KernelPatch

KPM-Manager depends on KernelPatch, inherits all its capabilities, and has been expanded.

You can install KernelPatch only, but this will not allow you to use Magisk modules

[Learn more about KernelPatch](https://github.com/yervant7/KPM-Manager)
...
- In addition, KPM-Manager directly utilizes magiskpolicy to provide additional SELinux support.

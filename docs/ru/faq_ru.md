...
## Что такое KPM-Manager?
KPM-Manager - это root решение, похожее на Magisk или KernelSU, которое объединяет лучшее из обоих.
Оно сочетает удобный и простой метод установки Magisk через `boot.img` с мощными возможностями исправления ядра KernelSU.

## В чем разница между KPM-Manager и Magisk?
- Magisk изменяет систему инициализации с помощью патча на RAM-диске вашего загрузочного образа, в то время как KPM-Manager вносит изменения непосредственно в ядро.


## KPM-Manager против KernelSU
- Для KernelSU требуется исходный код ядра вашего устройства, который не всегда предоставляется OEM-производителем. KPM-Manager работает напрямую с вашим исходным `boot.img`.


## KPM-Manager против Magisk, KernelSU
- KPM-Manager позволяет вам при желании не изменять SELinux. Это означает, что поток приложения может быть рутирован, в libsu и IPC нет необходимости.
...
Для получения дополнительной информации смотрите [Как написать KPM](https://github.com/yervant7/KPM-Manager/blob/main/doc/module.md)


## Связь между KPM-Manager и KernelPatch

KPM-Manager основан на KernelPatch, унаследовал все его возможности и был расширен.
...
[Узнать больше о KernelPatch](https://github.com/yervant7/KPM-Manager)
...
- KernelPatch не изменяет контекст SELinux и обходит SELinux с помощью перехвата.
  ...
  Это очень удобно.
- Кроме того, KPM-Manager напрямую использует magiskpolicy для обеспечения дополнительной поддержки SELinux.

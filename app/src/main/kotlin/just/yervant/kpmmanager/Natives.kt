...
object Natives {
    ...
    class KPMLoadRes {
        ...
        constructor(rc: Long, msg: String?) {
            this.rc = rc
            this.msg = msg
        }
    }
    ...
    fun loadKernelPatchModule(modulePath: String, args: String): KPMLoadRes {
        val s = KPMServiceConnection.service
        return if (s != null) {
            try {
                val bundle = s.nativeLoadKernelPatchModule(KPMMApplication.superKey, modulePath, args)
                KPMLoadRes(bundle.getLong("rc"), bundle.getString("msg"))
            } catch (e: Exception) {
                KPMLoadRes(-1L, e.message)
            }
        } else {
            KPMLoadRes(-1L, "Service disconnected")
        }
    }

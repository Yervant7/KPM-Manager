...
class HanziToPinyin protected constructor(private val mHasChinaCollator: Boolean) {
    ...
    private fun getToken(character: Char): Token {
        ...
        val pinyinBytes = PINYINS[offset]
        var j = 0
        while (j < pinyinBytes.size && pinyinBytes[j] != 0.toByte()) {
            pinyin.append(pinyinBytes[j].toInt().toChar())
            j++
        }
    ...
    /**
     * Convert the input to an array of tokens. The sequence of ASCII or Unknown characters without
     * space will be put into a Token, One Hanzi character which has pinyin will be treated as a
     * Token. If there is no China collator, the empty token array is returned.
     */
    ...
    companion object {
        ...
        fun getInstance(): HanziToPinyin {
            ...
            val locales = Collator.getAvailableLocales()
            for (locale in locales) {
                if (locale == Locale.CHINA || locale.language.contains("zh")) {
                    if (DEBUG) {

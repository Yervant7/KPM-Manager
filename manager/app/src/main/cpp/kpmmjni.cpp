...
jobject nativeLoadKernelPatchModule(JNIEnv *env, jobject /* this */, jstring super_key_jstr, jstring module_path_jstr,
                                  jstring args_jstr)
{
    ensureSuperKeyNonNull(super_key_jstr);
    ...
    struct kpm_load_result result_buf;
    memset(&result_buf, 0, sizeof(result_buf));
    result_buf.magic = KPM_LOAD_RESULT_MAGIC;
    result_buf.size = sizeof(result_buf);

    long rc = sc_kpm_load(super_key.get(), module_path.get(), args.get(), &result_buf);
    if (rc < 0) [[unlikely]] {
        LOGE("nativeLoadKernelPatchModule error: %ld", rc);
    }

    jclass cls = env->FindClass("just/yervant/kpmmanager/Natives$KPMLoadRes");
    jmethodID constructor = env->GetMethodID(cls, "<init>", "()V");
    jfieldID rcField = env->GetFieldID(cls, "rc", "J");
    jfieldID msgField = env->GetFieldID(cls, "msg", "Ljava/lang/String;");

    jobject obj = env->NewObject(cls, constructor);
    env->SetLongField(obj, rcField, rc);
    env->SetObjectField(obj, msgField, env->NewStringUTF(result_buf.message));
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void * /*reserved*/)
{
    ...
    const static JNINativeMethod gMethods[] = {
        ...
        { "nativeLoadKernelPatchModule", "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljust/yervant/kpmmanager/Natives$KPMLoadRes;",
